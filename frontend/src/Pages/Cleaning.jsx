...
import { assignWorkerToRoom, fetchAllhousekeepingrooms, fetchFreeWorker, approveCleaningRoom } from '../Redux/Slice/housekeepingSlice.js';
const Cleaning = () => {
                                                ...
                                                {visibleColumns.status && (
                                                    <td className="px-5 py-2 md600:py-3 lg:px-6">
                                                        <span className={`inline-flex items-center justify-center w-24 h-8 rounded-xl text-xs font-semibold ${getStatusStyle(housekeeping.status)}`}>
                                                            {housekeeping.status === 'Clean' ? 'Available' : housekeeping.status}
                                                        </span>
                                                        {/* Approve button for Completed cleaning */}
                                                        {housekeeping.status === 'Completed' && (
                                                          <button
                                                            onClick={async () => {
                                                              await dispatch(approveCleaningRoom(housekeeping.id));
                                                              dispatch(fetchAllhousekeepingrooms());
                                                            }}
                                                            className="ml-2 px-3 py-1.5 bg-green-500 text-white rounded hover:bg-green-600 text-xs font-semibold transition-colors"
                                                          >
                                                            Approve
                                                          </button>
                                                        )}
